...
FS.Cor <- cor(father.son$fheight, father.son$sheight)
b <- replicate(10000, {a <- sample_n(father.son, 100)
                    cor(a$fheight, a$sheight)-FS.Cor})
sd(b)
...
```{r}
FS.Cor <- cor(father.son$fheight, father.son$sheight)
c <- replicate(10000, {a <- sample_n(father.son, 250)
                    cor(a$fheight, a$sheight)-FS.Cor})
sd(c)
...
$$
\mbox{Var}(Y \mid X=x )= {1-\rho^2} \sigma_y
$$
