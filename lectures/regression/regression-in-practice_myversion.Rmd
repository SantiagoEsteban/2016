...
hist(dat$'2013')
hist(dat$'2014')
plot(dat$'2013', dat$'2014')

fit <- dat %>% do(augment(lm(`2014`~`2013`, data=.), data=.))
select(fit, nameFirst, nameLast, X2013, X2014, .fitted) %>% arrange(desc(X2013)) %>% tbl_df()
```
...
```{r}
Teams_since1961 <- filter(Teams, yearID >=1961) %>% select(teamID, R, BB, HR)
cor(Teams_since1961$R, Teams_since1961$BB)
cor(Teams_since1961$R, Teams_since1961$HR)
cor(Teams_since1961$BB, Teams_since1961$HR)
...
fit <- Teams_since1961 %>% filter(HR>=120 & HR<=130) %>% lm(R~BB, data=.)
tidy(fit)

fit2 <- Teams_since1961 %>% lm(R~BB, data=.)
tidy(fit2)
```
...
```{r}
Teams_since1961$HRdecile <- cut(Teams_since1961$HR, quantile(Teams_since1961$HR, probs=seq(0,1,0.1)), labels=FALSE)
ggplot(Teams_since1961, aes(x=BB, y=HR)) + geom_point() + facet_wrap(~HRdecile) + geom_smooth(method='lm')
