...
```{r}
X <- sample(c(-1,35), 10, replace = TRUE, prob=c(37/38, 1/38))
sum(X)
```
...
<<<<<<< HEAD
<<<<<<< HEAD
a(1) * the prob of that value +b(-1)*1-prob of a.

Assessment: What are the averages for the roulette examples? 
- Betting on black
> 1*(18/38)+(-1)*(20/38)
[1] -0.05263158
=======
...
>>>>>>> 715c7c1d9f420cf5fc3782ff3565ce01936884d9
- Betting on black 
>>>>>>> a29a6322134ef77787c14466ed7011e51e56f474
- Betting on 7
> 35*(1/38)+(-1)*(37/38)
[1] -0.05263158
...
```{r}
<<<<<<< HEAD
Z <- replicate(10000,{c <- sample(x,N,replace=TRUE) 
                        p_hat <- mean(c)
                        error <- p_hat - p})
hist(Z)
mean(Z)
...
Notice that the Monte Carlo simulation is like conducting 10,000 polls. This of course is practically impossible. But the mathematical theory saves us from having to do this! With just one poll we are able to say that our estimate is $\hat{p}$ and there is 32% chance that our error is larger than 2%. 
...
>>>>>>> 6bcbac7af4804930e8f789ae0678f83b2ded8a2b
...
we want the probability to be 0.001 or less. How big does N have to be.
N <- ceiling(-qnomr(1-10^-3*sigma/mu)^2)
3481 is the answer.

N <- ceiling (-wnomr(1-10^-3)*sigma/mu)^2
...
absolute(yhat-mu)/se > 1/(sigma/sqrt(n)
...
<<<<<<< HEAD
$\mbox{Pr}(\mid Z \mid \leq z) = 0.95$ ? 1.96
=======
...
<<<<<<< HEAD
4. Now use a Monte Carlo simulation to re run this experiment (keeping `prob_hear` fixed) 10000 times. Do we confirm that we cover 95% of the time? http://goo.gl/forms/GqHUcZkSDM
```{r}
