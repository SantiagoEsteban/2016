...
```{r}
thirty <- filter(tab, start_date>'2008-01-01' & week <= -30) %>% group_by(week) %>% mutate(n_polls=n()) %>% filter(n_polls>=5) %>% ungroup %>% summarize(theta_hat = mean(thirty$diff), sigma_hat = sd(thirty$diff), tot_poll=n())
...
```{r}
theta_hat + c(-1,1)*qnorm(0.975)*thirty$sigma_hat/sqrt(thirty$tot_poll)
...
```{r}
1 - pnorm(0, thirty$theta_hat, thirty$sigma_hat/sqrt(thirty$tot_poll))
...
The distribution between the pollsters is very different.

If you recall above, we discussed how to use the statistical 
...
```{r}
tab %>% 
    filter(start_date>='2008-01-01' & N == 1000) %>% summarize(theta = mean(diff), total_polls=n(), poll_size=unique(N), sigma=2*sqrt(mean(Obama)*(1-mean(Obama)))/sqrt(1000))
...
```{r}
tab_est <- tab %>% 
    filter(start_date>='2008-01-01' & N == 1000) %>% summarize(sigma_hat=sd(diff))
...
```{r}
N <- thirty$tot_poll
B <- (1/5^2)/(N/thirty$sigma_hat^2)+1/(1/5^2)
B*-0.15+(1-B)*thirty$theta_hat
...
biassd = 0.005
results <- replicate(B, {
    X <- mean( (rnorm(N,mu_post,sd_post) + 
                rnorm(1, 0, biassd))> 0)
})
qplot(results, geom = "histogram", bins = 20, 
      xlab = "Probability that Obama beats McCain", 
      main = "with a general poll bias") + 
    geom_vline(aes(xintercept = mean(results), color = "red")) + 
